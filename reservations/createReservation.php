...
$chambres = $conn->query("SELECT idChambre, numero, capacite FROM chambre WHERE disponibilite = 1 ORDER BY numero")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute([$idClient, $idChambre, $dateDebut, $dateFin])) {
            closeDatabaseConnection($conn);
            header('Location: listReservations.php?message=' . urlencode('SUCCÈS : Réservation créée avec succès.'));
            exit;
        } else {
            closeDatabaseConnection($conn);
            header('Location: listReservations.php?message=' . urlencode('ERREUR : Erreur lors de la création de la réservation.'));
            exit;
        }
    } else {
        $encodedMessage = urlencode("ERREUR : " . implode("<br>", $errors));
        header("Location: createReservation.php?message=$encodedMessage&idClient=$idClient&idChambre=$idChambre&dateDebut=$dateDebut&dateFin=$dateFin");
        exit;
    }
}
...
$selectedClientId = $_GET['idClient'] ?? '';
$selectedChambreId = $_GET['idChambre'] ?? '';
$dateDebutValue = $_GET['dateDebut'] ?? '';
$dateFinValue = $_GET['dateFin'] ?? '';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<body>
    ...
    <div class="container mt-5">
        ...
        <form method="post">
            ...
            <button type="submit" class="btn btn-success"><i class="fas fa-check me-1"></i> Créer</button>
