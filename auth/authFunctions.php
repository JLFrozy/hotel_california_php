...
function logoutUser() {
    ...
    header("Location: /HOTEL_CALIFORNIA_PHP/index.php?message=" . urlencode("SUCCÈS : Vous avez été déconnecté."));
    exit;
}
...
function requireRole($role = null) {
    ...
    if (!isLoggedIn()) {
        header("Location: /HOTEL_CALIFORNIA_PHP/auth/login.php");
        exit;
    }

    // Si un rôle est requis et que l'employe ne l'a pas, refuser l'accès
    if ($role !== null && !hasRole($role)) {
        $encodedMessage = urlencode("ERREUR : Accès refusé. Vous n'avez pas les permissions nécessaires.");
